...
import { StyleSheet, View, Text, TouchableOpacity, ScrollView, ActivityIndicator, Animated, Dimensions, SafeAreaView } from 'react-native';
...
const AdminVoiceAssistant: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const getEventsDeclaration: FunctionDeclaration = {
    ...
    parameters: {
      type: Type.OBJECT,
      description: 'Fetch current schedule. Essential for context before booking or cancelling.',
      properties: { timeMin: { type: Type.STRING }, timeMax: { type: Type.STRING } },
  ...
  const createEventDeclaration: FunctionDeclaration = {
    ...
    parameters: {
      type: Type.OBJECT,
      description: 'Pre-schedule a session. This triggers a confirmation request to Dawn.',
      properties: {
        clientName: { type: Type.STRING },
        service: { type: Type.STRING },
        startTime: { type: Type.STRING },
        endTime: { type: Type.STRING },
        summary: { type: Type.STRING, description: 'A brief summary of what you are doing.' }
      },
  ...
  const cancelEventDeclaration: FunctionDeclaration = {
    ...
    parameters: {
      type: Type.OBJECT,
      description: 'Remove a session. This triggers a confirmation request to Dawn.',
      properties: { 
        eventId: { type: Type.STRING },
        summary: { type: Type.STRING, description: 'Briefly state which event is being removed.' }
      },
  ...
  const confirmActionDeclaration: FunctionDeclaration = {
    ...
    parameters: {
      type: Type.OBJECT,
      description: 'Call this ONLY when Dawn explicitly confirms the previous request (e.g. says "Yes", "Do it", "Confirm").',
      properties: {},
      required: []
    }
  };
        ...
        callbacks: {
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            if (message.toolCall) {
              for (const fc of message.toolCall.functionCalls) {
                ...
                if (fc.name === 'getCalendarEvents') {
                  ...
                  setPendingAction({ type: 'cancel', data: { eventId: args.eventId }, summary: args.summary });
                  toolResult = "Cancellation queued.";
                } else if (fc.name === 'confirmPendingAction') {
                  if (pendingAction) {
                    if (pendingAction.type === 'create') await CalendarService.createEventSecure(pendingAction.data);
                    else if (pendingAction.type === 'cancel') await CalendarService.cancelEventSecure(pendingAction.data.eventId);
                    toolResult = "Executed.";
            ...
            if (message.serverContent?.turnComplete) {
              setTranscripts(prev => [
                ...prev,
                ...(currentInput ? [{ id: Date.now() + '-in', role: 'user', text: currentInput, isComplete: true } as TranscriptItem] : []),
                ...(currentOutput ? [{ id: Date.now() + '-out', role: 'model', text: currentOutput, isComplete: true } as TranscriptItem] : []),
              ]);
              setCurrentInput('');
              setCurrentOutput('');
            }
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.header}>
          ...
          <View style={styles.statusIndicator}>
            ...
            <Text style={styles.statusText}>{isActive ? 'LIVE' : 'CONNECTING'}</Text>
          </View>
        </View>
        ...
        >
          {transcripts.map((t) => (
            <View key={t.id} style={styles.narrativeItem}>
              <Text style={[styles.narrativeText, t.role === 'user' ? styles.userText : styles.modelText]}>
                {t.text}
              </Text>
            </View>
          ))}
          ...
          {currentInput ? (
            <View style={styles.narrativeItem}>
              <Text style={[styles.narrativeText, styles.userText, styles.streamingText]}>{currentInput}</Text>
            </View>
          ) : null}

          {currentOutput ? (
            <View style={styles.narrativeItem}>
              <Text style={[styles.narrativeText, styles.modelText, styles.streamingText]}>{currentOutput}</Text>
            </View>
          ) : null}

          {!currentInput && !currentOutput && transcripts.length === 0 && (
            <Text style={styles.placeholderText}>Start speaking to Becky...</Text>
          )}
        </ScrollView>
          ...
          {pendingAction && (
             <Animated.View style={styles.confirmBanner}>
                <View style={styles.bannerInfo}>
                  ...
                  <Text style={styles.bannerSummary}>{pendingAction.summary}</Text>
                </View>
...
const styles = StyleSheet.create({
  overlay: { ...StyleSheet.absoluteFillObject, backgroundColor: '#020617', zIndex: 1000 },
  safeArea: { flex: 1 },
  ...
  backText: { color: '#94A3B8', fontSize: 11, fontWeight: '800', textTransform: 'uppercase', letterSpacing: 2 },
  statusIndicator: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: 'rgba(255,255,255,0.05)', paddingHorizontal: 12, paddingVertical: 6, borderRadius: 100 },
  dot: { width: 6, height: 6, borderRadius: 3, backgroundColor: '#475569' },
  dotActive: { backgroundColor: '#14B8A6' },
  statusText: { color: '#94A3B8', fontSize: 10, fontWeight: '900', letterSpacing: 1 },
  
  transcriptStream: { flex: 1, paddingHorizontal: 40 },
  transcriptContent: { paddingVertical: 40 },
  narrativeItem: { marginBottom: 32 },
  narrativeText: { fontSize: 28, lineHeight: 42, fontWeight: '300', letterSpacing: -0.5 },
  userText: { color: '#94A3B8' },
  modelText: { color: '#F8FAFC' },
  streamingText: { opacity: 0.7 },
  placeholderText: { color: '#334155', fontSize: 24, fontWeight: '300', textAlign: 'center', marginTop: '20%' },

  bottomSection: { paddingBottom: 60, paddingHorizontal: 24 },
  confirmBanner: { backgroundColor: 'rgba(245, 158, 11, 0.1)', padding: 24, borderRadius: 32, borderLeftWidth: 4, borderLeftColor: '#F59E0B', marginBottom: 40, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  bannerInfo: { flex: 1 },
  bannerLabel: { color: '#F59E0B', fontSize: 10, fontWeight: '900', letterSpacing: 2, marginBottom: 4 },
  bannerSummary: { color: '#FDE68A', fontSize: 15, fontWeight: '500' },
  bannerPrompt: { color: '#F59E0B', fontSize: 10, fontWeight: '800', opacity: 0.6 },
