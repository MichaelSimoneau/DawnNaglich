...
const Booking: React.FC<BookingProps> = ({ activeSlotId, onSlotSelect }) => {
  ...
  const getTimeSlotIndex = (day: Date, time: string, slots: string[]): number => {
  ...
  const canExpandToSlot = (day: Date, time: string, slots: string[]): { canExpandUp: boolean; canExpandDown: boolean } => {
    if (selectedSlots.length === 0) return { canExpandUp: false, canExpandDown: false };
    
    const currentSlotIndex = getTimeSlotIndex(day, time, slots);
    if (currentSlotIndex === -1) return { canExpandUp: false, canExpandDown: false };
    ...
    if (selectedIndices.length === 0) return { canExpandUp: false, canExpandDown: false };
    ...
    const canExpandUp = currentSlotIndex === minSelected - 1 && 
                       selectedIndices.length < 3 && 
                       !isSlotTaken(day, time);
    
    // Can expand down if this slot is immediately after the last selected slot
    const canExpandDown = currentSlotIndex === maxSelected + 1 && 
                         selectedIndices.length < 3 && 
                         !isSlotTaken(day, time);
  ...
  const isSlotInSelectedRange = (day: Date, time: string, slots: string[]): boolean => {
    ...
    if (existingIndex !== -1) {
      ...
      if (selectedSlots.length === 0) {
        ...
        if (selectedIndices.length === 0) {
          ...
          if ((slotIndex === minSelected - 1 || slotIndex === maxSelected + 1) && selectedSlots.length < 3) {
            const newSlots = [...selectedSlots, { id, day, time }].sort((a, b) => {
              if (a.day.toDateString() !== b.day.toDateString()) {
                return a.day.getTime() - b.day.getTime();
              }
              const aMinutes = parseTimeToMinutes(a.time);
            ...
            */}
            ...
            {(() => {
              ...
              return slots.map(time => {
                ...
                const isInSelectedRange = isSlotInSelectedRange(day, time, slots);
                const isSlotSelected = isSlotInSelection(id);
                const { canExpandUp, canExpandDown } = canExpandToSlot(day, time, slots);
                ...
                return (
                  <View 
                    ...
                    style={[
                      ...
                      isInSelectedRange && !isSelected && styles.agendaRowInRange
                    ]}
                  >
                    ...
                    <TouchableOpacity 
                      ...
                      style={[
                        ...
                        (isSelected || isInSelectedRange) && styles.agendaRowHeaderSelected
                      ]}
                      activeOpacity={0.8}
                    >
                      <View style={styles.rowTimeBox}>
                        <Text style={[
                          ...
                          (isSelected || isInSelectedRange) && styles.textSelected
                        ]}>
                          {time}
                        </Text>
                      </View>
                      ...
                      <View style={styles.rowMain}>
                        {taken ? (
                          ...
                          <View style={styles.availableContainer}>
                            <Text style={[styles.availableLabel, (isSelected || isInSelectedRange) && styles.textSelected]}>
                              {isSelected ? 'Secure your session below' : 
                               isInSelectedRange ? `Part of ${selectedSlots.length * 30}-min session` :
                               canExpandUp || canExpandDown ? 'Click to extend session' :
                               `Book at ${time}`}
                            </Text>
                            <Text className={`fa-solid ${isSelected ? 'fa-chevron-up' : (isInSelectedRange ? 'fa-check' : 'fa-plus')} ${(isSelected || isInSelectedRange) ? 'text-emerald-900' : 'text-emerald-400'} text-[11px] ml-auto`} />
                          </View>
                        )}
                      </View>
                    </TouchableOpacity>
                    ...
                    {canExpandDown && (
                      <View style={[styles.expandIndicator, styles.expandIndicatorBottom]}>
                        <Text style={styles.expandIndicatorText}>↓ Expand</Text>
                      </View>
                    )}
                    ...
                    {isSelected && (() => {
                      ...
                      const timeRange = selectedSlots.length === 1 
                        ? firstSlot.time 
                        : `${firstSlot.time} - ${endTime.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true })}`;
                      const durationText = selectedSlots.length === 1 ? '' : ` (${selectedSlots.length * 30} min)`;
                        ...
                        <View style={styles.formContent}>
                          <View style={styles.formHeader}>
                            ...
                            <Text style={styles.formSub}>
                              Scheduling for {day.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} at {timeRange}{durationText}
                            </Text>
                          </View>
                      ...
                      <View style={styles.formActions}>
                        <TouchableOpacity 
                          style={[styles.submitBtn, (!formData.name || !formData.email || selectedSlots.length === 0) && { opacity: 0.5 }]} 
                          onPress={handleBookSubmit} 
                          disabled={isBooking || !formData.name || !formData.email || selectedSlots.length === 0}
                        >
                          {isBooking ? <ActivityIndicator color="#FFF" /> : <Text style={styles.submitBtnText}>Confirm Appointment</Text>}
                        </TouchableOpacity>

                        <TouchableOpacity style={styles.cancelBtn} onPress={() => toggleSlot(id, day, time, slots)}>
...
const styles = StyleSheet.create({
  ...
  agendaRowContainer: { overflow: 'hidden', borderBottomWidth: 1, borderColor: '#F1F5F9', position: 'relative' },
