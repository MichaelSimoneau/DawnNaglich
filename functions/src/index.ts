 ...
 */
export const getCalendarEventsSecure = onCall(async (request) => {
 ...
 */
export const createCalendarEventSecure = onCall(async (request) => {
 ...
 */
export const confirmCalendarEventSecure = onCall(async (request) => {
  const userEmail = request.auth?.token.email;
  const isAdmin = userEmail && ADMIN_EMAILS.includes(userEmail.toLowerCase());
 ...
 */
export const cancelCalendarEventSecure = onCall(async (request) => {
  const userEmail = request.auth?.token.email;
  const isAdmin = userEmail && ADMIN_EMAILS.includes(userEmail.toLowerCase());

  if (!isAdmin) {
    throw new HttpsError("permission-denied", "Only admins can cancel events.");
  }
