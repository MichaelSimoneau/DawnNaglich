          ...
          compiler.hooks.emit.tap('FixHtmlPaths', (compilation) => {
            ...
            Object.keys(compilation.assets).forEach((filename) => {
              if (filename.endsWith('.html')) {
                ...
                const absolutePathRegex = /src=["'](\/Users\/[^"']+node_modules[^"']+expo-router[^"']+entry\.js)["']/g;
                
                source = source.replace(absolutePathRegex, (match, absolutePath) => {
                  ...
                  const relativeFromRoot = absolutePath.replace(projectRoot, '');
...
function fixHtmlPathsAfterExport(outputDir = 'dist') {
  ...
  const possiblePaths = [
    path.join(distRoot, 'Users', 'devcoup', 'Projects', 'dawn-naglich', 'node_modules', '.pnpm', 'expo-router@3.5.24_d6c994eb312fe68e57c872410ae196eb', 'node_modules', 'expo-router', 'entry.js'),
  ...
  if (entryPath) {
    ...
    const webPath = '/' + relativePath.replace(/\\/g, '/').replace(/^\.\.\//, '');
